...
# Vẫn giữ danh sách này để làm tham số quét API, nhưng sẽ không lọc cứng ở vòng lặp trong
...
def fetch_final(target_name, endpoint, country_target=None, is_movie_logic=None):
    ...
    for year in YEARS:
        if len(results) >= TARGET_COUNT:
            break
        # Quét nhiều trang hơn để lấy phim thoải mái không lo về năm
        max_pages = 10 
        ...
        for page in range(1, max_pages + 1):
            ...
            for detail in details:
                ...
                m = detail['movie']
                m_year = m.get('year', 0) # Giữ info year
                ...
                # BỎ ĐIỀU KIỆN: (m_year == year) - Chỉ lọc quốc gia và loại phim
                if (not country_target or country_target in countries) and \
                   (is_movie_logic is None or is_movie == is_movie_logic):
                    ...
                    results.append({
                        "name": m.get('name'),
                        "year": m_year, # Vẫn xuất info year ra JSON
                        "slug": m.get('slug'),
...
def fetch_by_lang(lang_code, lang_name):
    results = []
    local_seen = set()
    print(f"> Đang quét: Phim {lang_name}...")
    ...
    for year in YEARS:
        ...
        for detail in details:
            ...
            results.append({
                "name": m.get('name'),
                "year": m.get('year', 0), # Giữ info year
                "slug": m.get('slug'),
                "thumb": m.get('thumb_url'),
                "poster": m.get('poster_url'),
                "sub_type": lang_name,
...
def interleave_trending(tr, han, au, thai, rap):
    ...
    l_han = list(han[:3])
    l_au = list(au[:3])
    l_thai = list(l_thai[:2]) if 'l_thai' in locals() else list(thai[:2])
    l_rap = list(rap[:3])
